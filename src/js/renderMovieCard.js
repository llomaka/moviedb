...
export function renderMovieCard(container, film, genres) {
  let genre = '';
  ...
  const markup = `<li class="movie__item">
    ...
    <div class="movie__caption">
      ...
      <p class="movie__genre">${genre} | ${release_date.slice(0, 4)} <span class="movie__vote">${film.votes_average.toFixed(1)}<span></p>
    </div>
  </li>
  `;
}
